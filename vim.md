...
The second approach is to type Ctrl-r then a character to paste the contents of the register identified by the character.
...
### visual block 模式
另一种是今天要讲的visual block模式，按下Ctrl-V进入
选择文本块后，键入"I"进入文本插入模式。键入你想输入的文本后，按下"Esc"，这样就会在你选择的文本块的每行的左边(选择文本的左边)全部插入你刚才输入的文本
